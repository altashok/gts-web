
export const enrollTranslations = {
  ta: {
    'enroll.title': 'மாணவர் சேர்க்கை',
    'enroll.subtitle': 'இன்றே பதிவு செய்யுங்கள், உங்கள் குழந்தையின் தமிழ்ப் பயணத்தைத் தொடங்குங்கள்.',
    'enroll.nextBatch.title': 'அடுத்த வகுப்புகள்',
    'enroll.nextBatch.desc': 'வருடம் முழுவதும் மாணவர் சேர்க்கை நடைபெறுகிறது. அடுத்த புதிய வகுப்புகளில் இணைய எங்களைத் தொடர்பு கொள்ளவும்.',
    'enroll.requirements.title': 'தேவைகள்',
    'enroll.requirements.point1': 'வயது 4 மற்றும் அதற்கு மேல்.',
    'enroll.requirements.point2': 'இணைய வசதி கொண்ட கணினி/டேப்லெட்.',
    'enroll.requirements.point3': 'ஆர்வமுள்ள மாணவர்கள்.',
    'enroll.requirements.point4': 'பெற்றோர் ஒத்துழைப்பு.',
    'enroll.form.studentName': 'மாணவர் பெயர்',
    'enroll.form.age': 'வயது',
    'enroll.form.parentName': 'பெற்றோர்/பாதுகாவலர் பெயர்',
    'enroll.form.email': 'மின்னஞ்சல்',
    'enroll.form.phone': 'தொலைபேசி எண்',
    'enroll.form.level': 'தற்போதைய நிலை',
    'enroll.form.skill': 'தேவைப்படும் வகுப்பு',
    'enroll.form.skill.placeholder': 'வகுப்பைத் தேர்ந்தெடுக்கவும்',
    'enroll.form.skill.options': ['தமிழ்', 'பரதநாட்டியம்', 'சங்கீதம்', 'வீரக்கலைகள்', 'வரைகலை', 'சதுரங்கம்', 'வேறு ஏதாவது'],
    'enroll.form.level.beginner': 'ஆரம்ப நிலை',
    'enroll.form.level.intermediate': 'இடைநிலை',
    'enroll.form.level.advanced': 'உயர் நிலை',
    'enroll.form.level.desc': 'உங்களுக்குத் தெரியாவிட்டால் ஆரம்ப நிலையைத் தேர்ந்தெடுக்கவும்.',
    'enroll.form.notes': 'கூடுதல் விவரங்கள்',
    'enroll.form.submit': 'இப்போதே பதிவு செய்யவும்',
    'enroll.form.disclaimer': 'உங்கள் விவரங்கள் பாதுகாப்பாக வைக்கப்படும்.',
  },
  en: {
    'enroll.title': 'Enrollment',
    'enroll.subtitle': 'Register today and start your child’s Tamil language journey.',
    'enroll.nextBatch.title': 'Next Batch',
    'enroll.nextBatch.desc': 'We enroll students throughout the year based on the age group and level. New classes / batches may start soon. Please talk to us for more information.',
    'enroll.requirements.title': 'Requirements',
    'enroll.requirements.point1': 'Ages 4 and above.',
    'enroll.requirements.point2': 'Basic understanding of English or Tamil',
    'enroll.requirements.point3': 'Parent/guardian support for younger children',
    'enroll.requirements.point4': 'Regular attendance and participation in classes',
    'enroll.form.studentName': 'Student Name',
    'enroll.form.age': 'Age',
    'enroll.form.parentName': 'Parent/Guardian Name',
    'enroll.form.email': 'Email Address',
    'enroll.form.phone': 'Phone Number',
    'enroll.form.level': 'Current Proficiency Level',
    'enroll.form.skill': 'Skill/Class',
    'enroll.form.skill.placeholder': 'Select a class',
    'enroll.form.skill.options': ['Tamil', 'Barathanatyam', 'Vocal', 'Martial Arts', 'Drawing', 'Chess', 'Something else'],
    'enroll.form.level.beginner': 'Beginner',
    'enroll.form.level.intermediate': 'Intermediate',
    'enroll.form.level.advanced': 'Advanced',
    'enroll.form.level.desc': 'Select beginner if you are unsure.',
    'enroll.form.notes': 'Additional Notes',
    'enroll.form.submit': 'Submit Enrollment',
    'enroll.form.disclaimer': 'Your data will be handled securely according to our privacy policy.',
  }
};
