"use client";

import Link from 'next/link';
import Image from 'next/image';
import { useState } from 'react';
import { Menu, X, Globe, ShoppingBag, ChevronDown } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useLanguage } from '@/context/LanguageContext';
import { cn } from '@/lib/utils';

type NavMenuItem = {
  id: string;
  label: string;
  href: string;
  children?: NavMenuItem[];
};

export default function Navbar() {
  const SHOP_DISABLED = true;
  const [isOpen, setIsOpen] = useState(false);
  const [openDropdown, setOpenDropdown] = useState<string | null>(null);
  const [mobileMenuOpenItem, setMobileMenuOpenItem] = useState<string | null>(null);
  const { language, setLanguage, t } = useLanguage();

  const logoImage = t('nav.logo') as string;
  const navMenuItems = (t('nav.menuItems') as NavMenuItem[]) || [];

  return (
    <>
      <div className="h-1.5 w-full bg-primary sticky top-0 z-[60]"></div>
      <nav className="bg-white/95 backdrop-blur-sm sticky top-1.5 z-50 border-b shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20">
            <div className="flex items-center">
              <Link href="/" className="flex items-center">
                <div className="relative h-16 w-40 sm:h-20 sm:w-52 md:h-20 md:w-46 shrink-0 pr-3 flex items-center justify-center p-1">
                  <Image 
                    src={logoImage} 
                    alt={t('nav.logoAlt') as string}
                    width={230}
                    height={80}
                    className="object-contain w-full h-full"
                    priority
                    data-ai-hint="school logo"
                  />
                </div>
              </Link>
            </div>

            {/* Desktop Navigation */}
            <div className="hidden md:flex items-center space-x-3 lg:space-x-4">
              {navMenuItems.map((item) => {
                if (item.children?.length) {
                  return (
                    <div
                      key={item.id}
                      className="relative py-2"
                      onMouseEnter={() => setOpenDropdown(item.id)}
                      onMouseLeave={() => setOpenDropdown(null)}
                    >
                      <button
                        type="button"
                        onClick={() => setOpenDropdown(openDropdown === item.id ? null : item.id)}
                        className={cn(
                          "flex items-center gap-1 font-normal hover:text-primary transition-colors whitespace-nowrap",
                          language === 'ta' ? 'text-[12px] tracking-tight' : 'text-[13px]'
                        )}
                      >
                        <span>{item.label}</span>
                        <ChevronDown className={cn("h-4 w-4 transition-transform", openDropdown === item.id && "rotate-180")} />
                      </button>

                      {openDropdown === item.id && (
                        <div className="absolute left-0 top-full w-48 rounded-md border border-border bg-white p-2 shadow-lg z-50">
                          {item.children.map((child) => (
                            <Link
                              key={child.id}
                              href={child.href}
                              onClick={() => setOpenDropdown(null)}
                              className={cn(
                                "block rounded-md px-3 py-2 text-sm font-normal text-foreground hover:bg-primary/10 hover:text-primary",
                                language === 'ta' ? 'text-[12px] tracking-tight' : 'text-[13px]'
                              )}
                            >
                              {child.label}
                            </Link>
                          ))}
                        </div>
                      )}
                    </div>
                  );
                }

                return (
                  <Link
                    key={item.id}
                    href={item.href}
                    className={cn(
                      "font-normal hover:text-primary transition-colors whitespace-nowrap",
                      language === 'ta' ? 'text-[12px] tracking-tight' : 'text-[13px]'
                    )}
                  >
                    {item.label}
                  </Link>
                );
              })}
              
              <div className="flex items-center border-l pl-2 lg:pl-5 space-x-1">
                {SHOP_DISABLED ? (
                  <Button
                    variant="ghost"
                    size="sm"
                    disabled
                    className="p-2 text-muted-foreground opacity-50 cursor-not-allowed"
                    title={`${t('nav.shop')} (temporarily disabled)`}
                  >
                    <ShoppingBag className="h-5 w-5" />
                  </Button>
                ) : (
                  <Button 
                    asChild
                    variant="ghost" 
                    size="sm" 
                    className="p-2 text-muted-foreground hover:text-primary"
                    title={t('nav.shop')}
                  >
                    <Link href="/shop">
                      <ShoppingBag className="h-5 w-5" />
                    </Link>
                  </Button>
                )}

                <Button 
                  variant="ghost" 
                  size="sm" 
                  onClick={(e) => {
                    setLanguage(language === 'ta' ? 'en' : 'ta');
                    e.currentTarget.blur();
                  }}
                  className="font-bold flex items-center gap-1 px-2 w-20"
                >
                  <Globe className="h-3.5 w-3.5" />
                  <span className="text-[12px]">{language === 'ta' ? 'English' : 'தமிழ்'}</span>
                </Button>
                
                <Button asChild className="bg-primary text-primary-foreground font-black hover:shadow-lg transition-all h-9 px-4 text-xs w-28">
                  <a href="https://portal.globaltamilschool.co.uk" target="_blank" rel="noopener noreferrer">
                    {t('nav.login')}
                  </a>
                </Button>
              </div>
            </div>

            {/* Mobile menu button */}
            <div className="md:hidden flex items-center space-x-3 sm:space-x-4">
              {SHOP_DISABLED ? (
                <Button
                  variant="ghost"
                  size="sm"
                  disabled
                  className="p-1 sm:p-2 text-muted-foreground opacity-50 cursor-not-allowed"
                  title={`${t('nav.shop')} (temporarily disabled)`}
                >
                  <ShoppingBag className="h-4 w-4 sm:h-5 sm:w-5" />
                </Button>
              ) : (
                <Button 
                  asChild
                  variant="ghost" 
                  size="sm" 
                  className="p-1 sm:p-2 text-muted-foreground hover:text-primary"
                >
                  <Link href="/shop">
                    <ShoppingBag className="h-4 w-4 sm:h-5 sm:w-5" />
                  </Link>
                </Button>
              )}
                <Button 
                  variant="ghost" 
                  size="sm" 
                  onClick={(e) => {
                    setLanguage(language === 'ta' ? 'en' : 'ta');
                    e.currentTarget.blur();
                  }}
                  className="font-bold p-1 sm:p-2 bg-accent text-accent-foreground hover:bg-accent hover:text-accent-foreground"
                >
                  <Globe className="h-4 w-4 sm:h-5 sm:w-5" />
                  <span className="text-[10px] sm:text-xs">{language === 'ta' ? 'EN' : 'TA'}</span>
                </Button>
              <button
                onClick={() => setIsOpen(!isOpen)}
                className="inline-flex items-center justify-center p-1 sm:p-2 rounded-md text-foreground hover:text-primary focus:outline-none"
              >
                {isOpen ? <X className="h-5 w-5 sm:h-6 sm:w-6" /> : <Menu className="h-5 w-5 sm:h-6 sm:w-6" />}
              </button>
            </div>
          </div>
        </div>

        {/* Mobile Navigation */}
        {isOpen && (
          <div className="md:hidden bg-background border-b animate-in slide-in-from-top-4 duration-200">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navMenuItems.map((item) => {
                if (item.children?.length) {
                  return (
                    <div key={item.id}>
                      <button
                        type="button"
                        onClick={() => setMobileMenuOpenItem((prev) => (prev === item.id ? null : item.id))}
                        className={cn(
                          "flex w-full items-center justify-between px-3 py-2 rounded-md font-bold text-foreground hover:bg-primary/10 hover:text-primary",
                          language === 'ta' ? 'text-sm' : 'text-base'
                        )}
                      >
                        <span>{item.label}</span>
                        <ChevronDown className={cn("h-4 w-4 transition-transform", mobileMenuOpenItem === item.id && "rotate-180")} />
                      </button>
                      {mobileMenuOpenItem === item.id && (
                        <div className="ml-3 mt-1 space-y-1">
                          {item.children.map((child) => (
                            <Link
                              key={child.id}
                              href={child.href}
                              className="block rounded-md px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-primary/10 hover:text-primary"
                              onClick={() => setIsOpen(false)}
                            >
                              {child.label}
                            </Link>
                          ))}
                        </div>
                      )}
                    </div>
                  );
                }

                return (
                  <Link
                    key={item.id}
                    href={item.href}
                    className={cn(
                      "block px-3 py-2 rounded-md font-bold text-foreground hover:bg-primary/10 hover:text-primary",
                      language === 'ta' ? 'text-sm' : 'text-base'
                    )}
                    onClick={() => setIsOpen(false)}
                  >
                    {item.label}
                  </Link>
                );
              })}
              <div className="pt-4 px-3 flex flex-col gap-2">
                <Button asChild className="w-full bg-primary text-primary-foreground font-black">
                  <a 
                    href="https://portal.globaltamilschool.co.uk" 
                    target="_blank" 
                    rel="noopener noreferrer"
                    onClick={() => setIsOpen(false)}
                  >
                    {t('nav.login')}
                  </a>
                </Button>
              </div>
            </div>
          </div>
        )}
      </nav>
    </>
  );
}
